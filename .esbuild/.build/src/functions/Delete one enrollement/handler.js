...
// src/functions/Delete one enrollement/handler.ts
__export(exports, {
  main: () => main
});
...
// src/functions/Delete one enrollement/handler.ts
var dlt_one_enrollement = async (event) => {
  try {
    ...
    const enrolled = await DynamoDB2.getData(query2);
    if (enrolled.Items.length > 0) {
      await DynamoDB2.deleteData(query);
...
var main = middyfy(dlt_one_enrollement);
