...
// src/functions/Update one course/handler.ts
__export(exports, {
  main: () => main
});
...
// src/functions/Update one course/handler.ts
var update_one_course = async (event) => {
  try {
    ...
    const course = await DynamoDB2.getData(query);
    if (course.Items.length > 0) {
      ...
      await DynamoDB2.updateData(query2);
...
var main = middyfy(update_one_course);
