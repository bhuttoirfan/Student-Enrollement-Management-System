...
// src/functions/Get List of enrollements/handler.ts
__export(exports, {
  main: () => main
});
...
// src/functions/Get List of enrollements/handler.ts
var get_list_of_enrollements = async (event) => {
  try {
    let enrollements = [{}];
    const query = {
      TableName: "SEMS"
    };
    const whole_data = await DynamoDB2.scanWholeData(query);
    whole_data.Items.map((cur) => {
      if (cur.courseid !== void 0) {
...
var main = middyfy(get_list_of_enrollements);
